...
class content extends getitems
{
    public function main_content()
    {
        ...
        // Change string variable according to error, if empty or invalid input//
        if (isset($_POST['save-product-btn'])) {

            $conn = $this->connect();
            $sku_exists = $_POST['sku'];
            $sku_duplicate = false;
            $sql =  mysqli_query($conn, "SELECT *  FROM stock WHERE SKU = '$sku_exists'");

            if (empty($_POST['sku'])) {
                $sku_err = '<span class="errors" id="sku_error">Please, submit required data</span><br>';
            } else if (preg_match('/^(?:\d+[a-zA-Z]|[a-zA-Z]+\d)[a-zA-Z\d]*$/', $_POST['sku'])) {
            ...
            } else if (preg_match('/\s*[^\S]/', $_POST['name'])) {
                $name_err = '<span class="errors" id="name_error">Please, submit required data</span><br>';
            }   else if (preg_match('/^[^\n]$/', $_POST['name'])) {
            ...
            } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,a-zA-Z]+$/', $_POST['price'])) {
        ...
        if (isset($_POST['save-product-btn'])) {
            if ($select === 'val1') {
                ...
                } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['size'])) {
            ...
            if ($select === 'val2') {
                ...
                } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['weight'])) {
            ...
            if ($select === 'val3') {
                ...
                } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['height'])) {
                ...
                } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['width'])) {
                ...
                } else if (!preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['length'])) {
        ...
        if (!empty($_POST['sku']) && !$sku_duplicate && preg_match('/^(?:\d+[a-zA-Z]|[a-zA-Z]+\d)[a-zA-Z\d]*$/', $_POST['sku']) && !empty($_POST['price']) && preg_match('/^[0-9]*\.?[0-9]*[^,a-zA-Z]+$/', $_POST['price']) && !empty($_POST['name']))
        {
            if ($select === 'val1' && !empty($_POST['size']) && preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['size'])) {
                $ins = new insertitems();
                $ins->insert();
            }
        }
        ...
        if (!empty($_POST['sku']) && !$sku_duplicate && preg_match('/^(?:\d+[a-zA-Z]|[a-zA-Z]+\d)[a-zA-Z\d]*$/', $_POST['sku']) && !empty($_POST['price']) && preg_match('/^[0-9]*\.?[0-9]*[^,a-zA-Z]+$/', $_POST['price']) && !empty($_POST['name']))
        {
            if ($select === 'val2' && !empty($_POST['weight']) && preg_match('/^[0-9]*\.?[0-9]*[^,[a-zA-Z]+$/', $_POST['weight'])) {
                $ins = new insertitems();
                $ins->insert();
            }
        }
        ...
        if (!empty($_POST['sku']) && !$sku_duplicate && preg_match('/^(?:\d+[a-zA-Z]|[a-zA-Z]+\d)[a-zA-Z\d]*$/', $_POST['sku']) && !empty($_POST['price']) && preg_match('/^[0-9]*\.?[0-9]*[^,a-zA-Z]+$/', $_POST['price']) && !empty($_POST['name']))
        {
            if ($select === 'val3' && !empty($_POST['width']) && !empty($_POST['height']) && !empty($_POST['length'])) {
                $ins = new insertitems();
                $ins->insert();
            }
        }
